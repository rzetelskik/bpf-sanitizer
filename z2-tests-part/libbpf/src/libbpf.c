...
BPF_PROG_TYPE_FNS(sanitizer, BPF_PROG_TYPE_SANITIZER);
...
static struct bpf_link *attach_sanitizer(const struct bpf_program *prog, long cookie);
...
static const struct bpf_sec_def section_defs[] = {
	...
	SEC_DEF("sanitizer/",			SANITIZER, BPF_SANITIZER, SEC_ATTACH_BTF, attach_sanitizer),
};
...
#define BTF_SANITIZER_PREFIX "bpf_sanitizer_"
...
struct bpf_link *bpf_program__attach_sanitizer(const struct bpf_program *prog)
{
	return bpf_program__attach_btf_id(prog);
}
...
static struct bpf_link *attach_sanitizer(const struct bpf_program *prog, long cookie)
{
	return bpf_program__attach_sanitizer(prog);
}
